...
from metahub.models.clients import (
    CategoryRead,
    ClientCreateUpdate,
    ClientRead,
...
async def register_client(
    client: ClientCreateUpdate, conn: Connection = Depends(get_db)
):
    result = await conn.fetchrow(
        "INSERT INTO clients (name, host) VALUES ($1, $2) RETURNING id",
        client.name,
        client.host,
    )
    if result is None:
...
async def register_category(
    client_id: int, category: CategoryCreateUpdate, conn: Connection = Depends(get_db)
):
    result = await conn.fetchrow(
        "INSERT INTO categories (name, client_id) VALUES ($1, $2) RETURNING id",
        category.name,
        client_id,
    )
    if result is None:
...
async def get_all_categories(conn: Connection = Depends(get_db)):
    ...
    return [
        CategoryRead(id=r["id"], name=r["name"], client_id=r["client_id"]) for r in rows
    ]
...
async def get_categories_of(client_id: int, conn: Connection = Depends(get_db)):
    rows = await conn.fetch(
        "SELECT id, name, client_id FROM categories WHERE client_id=$1", client_id
    )
    return [
        CategoryRead(id=r["id"], name=r["name"], client_id=r["client_id"]) for r in rows
    ]
...
async def delete_category(
    client_id: int, category_id: int, conn: Connection = Depends(get_db)
):
    await conn.execute(
        "DELETE FROM categories WHERE id=$1 AND client_id=$2", category_id, client_id
    )
...
async def modify_category(
    ...
    await conn.execute(
        "UPDATE categories SET name=$1 WHERE id=$2 AND client_id=$3",
        category.name,
        category_id,
        client_id,
    )
