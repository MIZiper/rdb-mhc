...
async def lifespan(app: FastAPI):
    # podman run -it --rm -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=password -e POSTGRES_DB=metahub -v $(pwd)/metahub/db/schema.sql:/docker-entrypoint-initdb.d/schema.sql -p 5432:5432 postgres:18
    await init_pool()
    yield
    await close_pool()
